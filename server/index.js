...
app.post("/api/presign/put", async (req, res) => {
	try {
		...
		const putUrl = await new Promise((resolve, reject) => {
			minioClient.presignedUrl("PUT", BUCKET, key, expires, (err, url) => (err ? reject(err) : resolve(url)));
		});
...
app.get("/api/presign/get", async (req, res) => {
	try {
		...
		const getUrl = await new Promise((resolve, reject) => {
			minioClient.presignedUrl("GET", BUCKET, key, expires, (err, url) => (err ? reject(err) : resolve(url)));
		});
...
app.get("/download", async (req, res) => {
	...
	try {
		const srcUrl = await new Promise((resolve, reject) => {
			minioClient.presignedUrl("GET", BUCKET, key, 3600, (err, url) => (err ? reject(err) : resolve(url)));
		});
...
app.get("/dl", async (req, res) => {
	...
	try {
		...
		const proxyUrl = `${
			process.env.PUBLIC_BASE_URL || "https://mirror-draw.metasoftco.com"
		...
		const html = tpl.replaceAll("{{VIEW_URL}}", viewUrl).replaceAll("{{PROXY_URL}}", proxyUrl);
...
// --------- Socket.IO ----------
const allowedOrigin = process.env.ALLOWED_ORIGIN || process.env.NEXT_PUBLIC_SITE_URL || "*";
		...
		try {
			...
			await new Promise((resolve, reject) => {
				...
				const args = [
					...
					framePath, // 1: PNG
					"-filter_complex",
					"[0:v]hflip,transpose=2,format=rgba[base];" +
						"[1:v]transpose=2[fg0];" +
						"[fg0][base]scale2ref=flags=lanczos[fg][base2];" +
						"[fg]format=rgba[fg2];" +
				...
				// NOT: execFile yerine spawn kullanıyoruz, buffer yok, stream var
				const child = spawn("ffmpeg", args, { stdio: ["ignore", "pipe", "pipe"] });

				// İsteğe bağlı: daha az gürültü için -hide_banner ve -loglevel info/error argümanlarına geçebilirsiniz.

				// Canlı log
				child.stdout.on("data", (d) => console.log(`[ffmpeg:out] ${d}`));
				child.stderr.on("data", (d) => console.log(`[ffmpeg:err] ${d}`));

				// Güvenlik: örn. 3 dakikayı geçerse öldür
				const KILL_MS = 3 * 60 * 1000;
				const killer = setTimeout(() => {
					console.warn("ffmpeg timeout – process kill denendi");
					child.kill("SIGKILL");
				}, KILL_MS);
				...
				child.on("close", (code, signal) => {
					...
					if (code === 0) {
						...
						const err = new Error(`ffmpeg exited with code=${code} signal=${signal || "none"}`);
						console.log("ffmpeg close non-zero", { code, signal });
						reject(err);
					}
				});
			});
